...
- 2026-07-02: `dev` was rebased from upstream stable `v0.346.18` onto `v0.349.5`; package metadata is aligned to `0.349.5`, and the upstream plain tag remains the release baseline at `13949c921130`.
- Rebase conflicts occurred in `core/session-coordinator.ts`, `tests/session-meta-write-serialization.test.ts`, `core/local-provider-plugin-store.ts`, `desktop/src/react/settings/tabs/providers/ProviderModelList.tsx`, and `desktop/src/react/settings/tabs/providers/__tests__/ProviderModelList.test.tsx`.
- Resolution preserved upstream aggregate `session-meta` budget externalization and discovered provider-model metadata, while keeping fork forced legacy sidecar externalization, memory-reflection sidecar hydration, replacement-list model deletion semantics, and encoded provider-model DELETE routing.
...
- Additional gates passed: conflict-focused `npx vitest run` set with 167 tests, `npm run typecheck`, `git diff --check`, and `node scripts/sync-upstream.mjs --conflict-plan --json --local-only`.
- Tier 3A local desktop install/version verification passed: `/Applications/HanaAgent.app` was rebuilt with `SKIP_NOTARIZE=true npm run install:local`, codesign verified, `CFBundleShortVersionString`, `CFBundleVersion`, and `build-info.json.appVersion` all reported `0.349.5`; `build-info.json` reported `channel: local`, `sourceRepo: karlorz/openhanako`, `baseTag: v0.349.5`, `dirty: false`, `updateEnabled: false`, and `signatureKind: adhoc`.
- Tier 3B live sg01 smoke passed: the helper restart/verify path returned identity HTTP 200 and WebSocket open; CDP UI smoke pasted/sent a generated `Pasted image.png`, switched to `Recent Changelogs Guide`, returned to `Image Preview Smoke Test`, confirmed the transcript image and Conversation Files row still rendered, then opened MediaViewer from Conversation Files and loaded the image from a scoped remote `/api/resources/res_sf_.../content` URL. No CSP refusal, WebSocket disconnect, or new smoke-image load failure was observed. One stale 410 for an older historical resource in the same smoke session was observed separately and did not affect the new upload/preview path.
