...
- 2026-07-04: `dev` was rebased from upstream stable `v0.349.5` onto `v0.350.2`; package metadata is aligned to `0.350.2`, and the upstream plain tag remains the release baseline at `2c3691c31043`.
- No manual rebase conflicts occurred. The upstream delta absorbed 8 non-merge commits across 48 files, including the MinGit runtime switch, desktop slash-command wiring, editor find focus preservation, spellcheck disabling on text surfaces, MCP capability drift preservation, and stream-end stabilization.
...
- Additional gates passed: `npx vitest run tests/sync-upstream.test.mjs`, `npm run typecheck`, `git diff --check`, and `node scripts/sync-upstream.mjs --conflict-plan --json --local-only`.
- Tier 3A local desktop install/version verification passed: `/Applications/HanaAgent.app` was rebuilt with `SKIP_NOTARIZE=true npm run install:local`, codesign verified, `CFBundleShortVersionString`, `CFBundleVersion`, and `build-info.json.appVersion` all reported `0.350.2`; `build-info.json` reported `channel: local`, `sourceRepo: karlorz/openhanako`, `baseTag: v0.350.2`, `gitSha: f2138867d7c9`, `dirty: false`, `updateEnabled: false`, and `signatureKind: adhoc`.
- Tier 3B live sg01 smoke passed: the helper restart/verify path returned identity HTTP 200 and WebSocket open; CDP UI smoke uploaded and sent `openhanako-sync-smoke-v0.350.2.jpg`, switched to `Recent Changelogs Guide`, returned to `Image Preview Smoke Test`, confirmed the new smoke turn and Conversation Files row still rendered, then opened MediaViewer from Conversation Files and loaded the image at `1024x1024` from scoped remote `/api/resources/res_sf_2a746d039596aeca/content?...` with no CSP refusal, WebSocket disconnect, or fresh preview failure.
- Fork release tag target for this sync is `v0.350.2-karlorz.1`; the plain upstream tag `v0.350.2` must not be pushed to `origin`.
