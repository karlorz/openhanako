...
All 12 pre-existing CI failures were then fixed and verified green on
both platforms: push CI run `31386628984` and PR #1 dashboard CI run
`31386631862` both completed with all four jobs (macOS tests, Windows
tests, open-boundary lint, open composition build smoke) passing.
Windows-only `model-manager-auth-storage` failure was traced to a stale
mtime-keyed provider-catalog cache after the direct store save in
`migrateLegacyApiKeyAuthToProviders` (fixed by invalidating the registry
cache, matching the existing pattern in `core/migrations.ts`); a
temporary `writeSecretFileSync` rename fallback was reverted because it
violated the documented throw-and-preserve contract. The read-only
local-only conflict plan currently reports zero conflicts and
`prUpdated: false` and `stableSyncAvailable: false`. `origin/dev` is now
synced with local `dev` (zero divergence). The fork-qualified
`v0.446.6-karlorz.1` digest alignment, immutable tag, and non-draft
legacy-raw release (20 assets, Build run `31376086862`) are published;
PR #1 remains the permanent draft dashboard, never-merge, no auto-merge.
...
| 2026-08-24 | `v0.449.0` | 5 conflicts across 4 files: `tests/windows-installer-contract.test.ts` (upstream seed-ordering test vs fork legacy-raw seed-resource test), `build/installer.nsh` (upstream seed-versioning comment vs fork dual-profile comment), `build/cli-runtime-closure.json` (generated receipt), `build/persistence-schema-fingerprint.json` (generated receipt), `release-digest.v1.json` + `release-digest.v2.json` (fork v0.446.6-karlorz.1 digest alignment vs upstream stable). | Preserved both test cases in `windows-installer-contract.test.ts`; preserved both comments in `installer.nsh` (fork dual-profile + upstream seed versioning); took fork side for generated receipts (regenerated during `--post-rebase`); took upstream stable for release digests v1/v2 per TAKE-UPSTREAM-STABLE-THEN-FORK-ALIGN policy. Upstream delta: 16 commits / 124 files — DeepSeek vision model support, beta-channel updater invite gate, AGENTS.md persona adoption, persistence store recovery, Windows installer seed fix, early-access npm package tracking. | `--post-rebase` Tier 0 passed (all 86 fork-only file patterns present); Tier 1 passed (16 focused suites: server-auth, server-connection, csp-sync, ws-message-handler, resource-url, chat-slice, file-refs, user-attachment-media, media-source, open-media-viewer, RightWorkspacePanel, upload-route, plugin-sdk-examples, office-workflow-plugin, MainContent.drag, InputArea.paste-and-slash + media-send); Tier 2 passed (`connect:probe` in main bundle, `probeConnection` in preload bundle, scoped remote resource-origin CSP logic present). | **Not run** — Tier 3A (local install/codesign/build-info at `0.449.0`) and Tier 3B (sg01 live smoke) are pending manual operator verification. | Backup `backup/dev-before-stable-v0.449.0-20260824` @ `b9baa235d`; `dev` rebased to `296d6a312` at package `0.449.0`. No push, tag, release, deployment, PR #1 mutation, or GitHub mutation. Release digests v1/v2 are upstream-owned `0.449.0`; fork-qualified digest alignment deferred until after Tier 3 verification. PR #1 remains permanent draft never-merge. |
| 2026-08-24 | `v0.449.0` receipt regeneration | CI push run `32649594032` failed: `tests/persistence-schema-tripwire.test.ts` reported fingerprint mismatch (committed `sha256:968b647f...` from v0.446.6 vs generated `sha256:fc1e4cac...` from v0.449.0 effective tree). `--post-rebase` does NOT regenerate persistence receipts — it only runs Tier 0-2 tests and bundle greps. | Regenerated fingerprint via `node scripts/generate-persistence-schema-fingerprint.mjs --classification compatible --compatibility-reason "Upstream v0.449.0 adds automation store failure recovery and cron-store recovery write points — compatible addition with no DATA_EPOCH change."`; regenerated CLI closure via `node scripts/compute-cli-closure.mjs` (9,690 files, was 9,686). Tripwire 18/18 green, typecheck clean. Committed as `767e206f7` and pushed to `origin/dev`. | Tripwire 18 tests passed; typecheck passed; CI push run `32650843580` in progress after re-push. | Not re-run — Tier 3A/3B still pending. | `dev` at `767e206f7` pushed to `origin/dev`. Concept page: `concepts/2026-08-24-openhanako-persistence-receipt-regeneration-after-rebase`. PR #1 remains permanent draft never-merge. |
