...
- 2026-06-25: `dev` was rebased cleanly from the `v0.341.19` baseline onto upstream stable `v0.345.3`; package metadata is aligned to `0.345.3`, and `package-lock.json` root-version alignment came from the attended stable sync.
- `node scripts/sync-upstream.mjs` and `node scripts/sync-upstream.mjs --post-rebase` both passed Tier 0 fork-only file presence, Tier 1 focused tests, and Tier 2 bundle greps for `connect:probe`, `probeConnection`, and scoped runtime CSP remote resource-origin logic.
- The read-only precheck reported conflict-likely changes in `desktop/src/react/utils/preview-document-refresh.ts`, `package.json`, `package-lock.json`, and `tests/plugin-sdk-examples.test.ts`, but the rebase itself completed without manual conflicts.
- Additional fork-sync gates passed: `npx vitest run tests/sync-upstream.test.mjs` (18 tests), `node scripts/sync-upstream.mjs --conflict-plan --json --local-only`, `npm run typecheck`, and `git diff --check`.
- Tier 3A local desktop install/version verification passed: `/Applications/HanaAgent.app` was rebuilt with `SKIP_NOTARIZE=true npm run install:local`, codesign verified, `CFBundleShortVersionString`, `CFBundleVersion`, and `build-info.json.appVersion` all reported `0.345.3`; `build-info.json` reported `channel: local`, `sourceRepo: karlorz/openhanako`, `baseTag: v0.345.3`, `dirty: false`, `updateEnabled: false`, and `signatureKind: adhoc`.
- Tier 3B live sg01 smoke passed: the helper restart/verify path returned identity HTTP 200 and WebSocket open; CDP UI smoke pasted/uploaded `Pasted image.png`, sent it, switched chats and returned, then confirmed the chat image and Conversation Files preview still rendered from the remote `/api/resources/res_sf_.../content` URL. No CSP refusal or WebSocket disconnect was observed during the check.
