...
- 2026-07-19: fork prerelease `v0.407.15-karlorz.6` was published from exact commit `7365233a0d0a36c467e3a97d9243f4620e80562a`. GitHub Actions run `29688911528` completed successfully. The release remains a prerelease, targets `dev`, and contains the expected 20-asset legacy-raw surface: seven desktop installers, five standalone server bundles, five SHA-256 sidecars, one compatibility manifest, one immutable profile marker, and `release-digest.v1.json`.
- Signed-only assets are intentionally absent: no `latest*.yml`, blockmaps, renderer/server hot-update archives, seed/train assets, AtomGit mirror output, or `release-digest.v2.json` release asset. The committed v1/v2 digests both validate, but the binding legacy-raw upload policy publishes v1 only.
...
- The configured digest gateway repeatedly returned HTTP 200 with no choices, so publication used the documented deterministic/manual fallback: a fresh source packet, the reviewed prior digest as a factual base, exact `.6` identity/signing updates, both schema validators, and independent provenance review. No API key was printed or committed.
- The arm64 DMG checksum matched GitHub (`977e705799db686d3038df30fa7a20b8aecbba7857db840f012d641f1f9cc6a6`). `/Applications/HanaAgent.app` now reports app version `0.407.15`, release tag `v0.407.15-karlorz.6`, git SHA `7365233a...`, profile/signature kind `legacy-raw`, and updater/artifact updates disabled; strict codesign verification passed.
- sg01 was upgraded through the supported `scripts/install-server.mjs` flow from `v0.346.18-karlorz.6-linux-arm64`. The selected `.6` server archive independently matched checksum `3e11ec6b5232af31df29af4c25ac47db14c1bbdf650b428a5555cddbeccdbd62`; execution reported `ok: true` and `rolledBack: false`, preserved the existing root/HANA_HOME service context, and activated `/opt/hanaagent/releases/v0.407.15-karlorz.6-linux-arm64`.
- Review then found sg01's durable `/usr/local/bin/install-server` still pointed at the older single-file CLI. The corrected post-release bootstrap was staged temporarily, fetched `install-server.mjs` plus its six shared status dependencies from the exact immutable `.6` tag, and installed dependencies before the CLI. Both `status --json` and `status --check-updates --json` now exit successfully. The immutable `.6` tag was not moved. The follow-up bootstrap on `dev` also detects whether the downloaded installer imports the shared modules, so historical single-file tags such as `v0.357.17-karlorz.1` remain bootstrappable while current refs still fail closed on a missing dependency closure.
- Fresh post-deploy evidence shows `hanaagent` active and enabled, `/mobile/`, `/mobile/locales/zh.json`, and `/mobile/locales/en.json` HTTP 200, identity HTTP 200, WebSocket open, exact release match, eligible deployability, valid compatibility manifest, no release-policy drift, and declared `chat.core@1`, `input.drafts@1`, and `websocket.ticket@1` contracts. The refreshed CLI's online status check independently reports `.6` as current and supplies the exact pinned prerelease dry-run command.
- The live desktop smoke uploaded and sent `codex-clipboard-a8d3eb49-de5c-48c7-8411-3c178b813212.png`, switched to another chat, returned to the new `.6` chat, and confirmed both the persisted transcript turn and Conversation Files row. The remote Conversation Files preview rendered completely at `834x775`; the attached browser console contained zero errors and zero warnings.
- Upstream issue search was refreshed on 2026-07-19: #1749 remains open, #1811 remains closed, and no exact matches were found for the pending local drafts. The tracker remains status/search/draft only; no issue was submitted.
- PR #1 remains open and draft and was not merged, closed, auto-merged, or used as a release vehicle. The immutable `.6` tag was not moved. Ignored UAT media and unrelated dirty SkillWiki work were not absorbed.
