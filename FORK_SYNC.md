...
- 2026-07-11: local `main`, the refreshed local `origin/main` tracking ref, `upstream/main`, and prerelease tag `v0.380.10` point at `7930f8670b6b`; no remote fork ref was pushed. GitHub still marks `v0.357.17` as the latest non-prerelease release and `v0.380.10` as a prerelease. No production sync is available yet.
...
- The refreshed read-only merge plan predicts seven textual conflicts: `desktop/main.cjs`, `desktop/preload.cjs`, `desktop/src/react/__tests__/components/InputArea.media-send.test.tsx`, `desktop/src/react/__tests__/components/MainContent.drag.test.tsx`, `desktop/src/react/settings/widgets/KeyInput.tsx`, `desktop/src/react/types.ts`, and `release-digest.v1.json`. An independent changed-path intersection remains 42 paths.
...
- Upstream has touched the LAN auth, Electron main/preload, scoped CSP, remote attachment/resource, WebSocket session identity, package metadata, upload route, and focused test surfaces since `v0.357.17`. The `v0.380.4..v0.380.10` additions also add Pi-native/manual compaction routing and a signed artifact-core activation library; these join the session/replay and installer/checksum surfaces that require semantic review even where merge-tree reports no textual conflict.
- Candidate-specific issue evidence now includes open reports for new-chat send gating (#2101), provider API-type changes not applying (#2103), slash/IME input state loss (#2104), cache-prefix failure after a user pause (#2105), and cross-Agent reminder leakage (#2106). #2096 is closed against the latest test build, but #2100 shows GPT-5.6 model availability remains inconsistent and must be verified against the eventual stable target.
