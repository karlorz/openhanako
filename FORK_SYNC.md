...
- 2026-06-27: `dev` was rebased cleanly from the `v0.345.3` baseline onto upstream stable `v0.346.18`; package metadata is aligned to `0.346.18`, and the lockfile root metadata came from the attended stable sync.
...
- The rebase completed without textual conflicts, but the semantic `desktop/src/react/services/resource-url.ts` review caught upstream's owner-only `isLocalOwnerConnection(connection)` transport check. Follow-up commit `a1fa7dde` restored the fork `connection.kind === 'local'` invariant and added LAN device-credential regression coverage.
- Additional fork-sync gates passed: `npx vitest run tests/sync-upstream.test.mjs` (19 tests), focused preview/resource tests (45 tests), `npm run typecheck`, `git diff --check`, and `node scripts/sync-upstream.mjs --conflict-plan --json --local-only`.
