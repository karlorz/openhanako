...
describe("HanaEngine marketplace skill package wiring", () => {
  it("migrates known legacy opt-outs and exposes source-qualified package gate state", () => {
    ...
    try {
      ...
      const service = {
        ...
        retireLegacyMarketplaceSkillOverrides: vi.fn(() => ({
          revision: 1,
          retiredLegacyRefs: [{ agentId: "agent-a", legacyRef: "wiki-query@llm-wiki/skillwiki" }],
        })),
      };
      ...
      expect(service.retireLegacyMarketplaceSkillOverrides).toHaveBeenCalledWith([
  ...
  it("removes the legacy registry override so direct activation is enabled after re-enable and reload", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "hana-engine-marketplace-skills-"));
    try {
      const skillsDir = path.join(home, "skills");
      writeSkill(path.join(skillsDir, "wiki-query"), "wiki-query");
      ...
      service.registry.setControlPlaneActivations({
        marketplaceSkills: {
      ...
      expect(service.registry.getControlPlaneActivations()).toEqual({
        marketplaceSkills: {
          "wiki-query@llm-wiki/skillwiki": { enabled: true },
        },
      });
      ...
      expect(service.getMarketplaceSkillActivation(
        "wiki-query",
        "llm-wiki",
        "skillwiki",
        { agentId: "agent-a" },
      )).toMatchObject({
        identity: "wiki-query@llm-wiki/skillwiki",
        state: "enabled",
        enabled: true,
        requested: true,
      });

      const reloadResolver = setResolver.mock.calls[setResolver.mock.calls.length - 1][0];
      expect(reloadResolver("wiki-query")).toEqual({
        identity: "skillwiki@llm-wiki",
        skillName: "wiki-query",
        enabled: true,
        state: "enabled",
