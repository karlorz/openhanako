...
- User resource access through runtime `ctx.resources`, with ResourceRef inputs for local files, mounts, SessionFiles, durable resources, and read-only URLs.
...
- User resource access must go through `ctx.resources`. Declare `resource.read` for `stat/read/list`, `resource.search` for search, `resource.write` for `write/writeExpectedVersion/edit/mkdir/delete/copy/rename/move/trash`, `resource.materialize` for concrete local paths, and `resource.watch` for watch-target resolution. URL resources are read-only. Do not use local path writes for user resources.
