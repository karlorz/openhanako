...
Current state (2026-07-27): package version remains **`0.416.51`**. Host **sg01** runs prerelease **`v0.416.51-karlorz.3`** (`c1658b34…`); Mobile session-loss recovery UAT-01…09 **PASS** (HTTP `/mobile/`). Earlier line: stable-first sync to `v0.416.44` then prerelease rebase onto `train-beta-17` / `v0.416.51` (upstream `ef8a6f70…`); fork tags `v0.416.44-karlorz.1` and `v0.416.51-karlorz.{1,2,3}` published as prereleases (`legacy-raw`). PR #1 remains permanent draft never-merge. **Open follow-ups:** HTTPS/WSS for true installed-PWA SW coverage; ship packaged `hana` CLI `shared/artifact-core` fix (`eed8ffc4` on `dev`, not yet on host until next prerelease). Vault work under `projects/openhanako/work/2026-07-26-mobile-pwa-session-loss-recovery/` and `2026-07-27-*`.
