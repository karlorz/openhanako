...
Current pre-sync state (2026-07-21): the fork remains on upstream stable `v0.407.15` (package `0.407.15`, latest fork prerelease `v0.407.15-karlorz.7`). No newer GitHub non-prerelease release exists, so production sync remains inactive. Upstream review-only ceiling from `--include-prerelease --check` is still `train-13`; mirrored `origin/main`/`upstream/main` moved to `8a3cbbfc` (package tip `0.415.15`, tag `v0.415.15` without a published GitHub release). Dry-run dashboard: **16** planned conflict paths (4 preserve-both, 10 take-main including new `core/session-turn-actions.ts`, 1 human-review on `desktop/main.cjs`, 1 deferred `package.json`), **57** risky overlapping paths, 10 migration contracts with `stableActivationAllowed: false`. See `FORK_SYNC.md`. Pick-from-main decisions are enforced via `docs/fork-sync/pick-from-main-decisions.yml` (see also `.md`); reclaim guards block duplicate re-apply of ticket-primary WS, connect-probe isolation, resource-url, and packaged-boot seams. No safe adopt-now while main tip is unreleased train content. Do not start a production sync or bump package metadata until a new stable release exists.
