...
Current state (2026-07-23): `dev` has been rebased onto upstream prerelease **`train-beta-15`** / paired release **`v0.416.43`** at upstream SHA `a02622da02cd2edc8397066c6cb6bc256f306b97`. Package, lockfile, and release-digest metadata align to `0.416.43` / `v0.416.43`; backup `backup/dev-before-prerelease-v0.416.43-20260723` preserves commit `61b3c8549d3e9332852eaa77e5ab01a0346b3d3c`. Local macOS `/Applications/HanaAgent.app` is rebuilt and strictly codesigned as signed local `0.416.43`; sg01 remains unchanged on `v0.412.7-karlorz.2-linux-arm64` (service active). Desktop smoke helper: identity 200 + WS open, functional pass, environment `attention` because the remote release is older than the installed client. Manual CDP smoke confirmed upload, attachment chips, Conversation Files persistence/preview, and an older chat thumbnail after switching chats; the newly captioned smoke message was not counted as persisted because it was not reverified after reload. No tag, GitHub release, or host deployment was authorized. See `FORK_SYNC.md` sync log. PR #1 remains permanent draft never-merge.
