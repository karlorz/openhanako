...
Current state (2026-07-23): `dev` is at `b385e5fb56bdfcbc45871becfd7d732ac17086d6` and is pushed to `origin/dev`. The immutable fork prerelease `v0.416.43-karlorz.1` is published from that SHA with the `legacy-raw` profile; tag-triggered Build run `29999546058`, push CI run `29996156803`, and permanent-dashboard PR CI run `29996159001` all passed. The exact published macOS arm64 DMG is installed at `/Applications/HanaAgent.app` with strict deep codesign and release metadata matching the tag/SHA. sg01 now points at `/opt/hanaagent/releases/v0.416.43-karlorz.1-linux-arm64`; `hanaagent` is active/enabled and the remote identity reports the exact release, SHA, repository, Linux arm64 platform, and current freshness. The helper reports functional pass with identity HTTP 200 and WebSocket open. Manual Computer Use smoke uploaded and sent `markdown-document-icon.png` with caption `release v0.416.43-karlorz.1 image smoke`, confirmed the transcript thumbnail and Conversation Files row, rendered a nonblank Media preview, switched to another chat, returned, and confirmed the file/thumbnail/preview again. The final local-only conflict plan observed upstream stable `v0.416.44` available; no stable sync was started in this prerelease closeout. PR #1 remains permanent draft never-merge. The release-created-by-bot event produced no AtomGit mirror run because the mirror workflow excludes `github-actions[bot]`; legacy-raw also intentionally excludes mirror/train assets. See `FORK_SYNC.md` for the release closeout.
