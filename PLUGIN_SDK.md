...
- Marketplace metadata lives outside the app repo in `OH-Plugins`. Inspect destination and adapter before choosing a lifecycle. `hana-skills` + `skill-manager` packages install as Hana skills, appear in Manage Plugins with a Hana skills badge, and use a global Marketplace package gate plus separate Agent Skill Toggles. `native-plugin` + `plugin-manager` packages use this SDK and PluginManager; native installation is preview-only/deferred in the current durable multi-source/Agent contract. Legacy single-catalog compatibility may still download a native `distribution.kind = "release"` zip, verify `sha256`, and record it in `${HANA_HOME}/plugin-installs.json`, but that path must not be generalized to Hana skill packages.
