...
export const PERSISTENT_STORES: readonly StoreDescriptor[] = Object.freeze([
  ...
  defineStore({
    ...
    pathPatterns: [
      ...
      "plugin-marketplace-cache/{sourceId}",
      "plugin-marketplace-git/{sourceId}",
      "plugin-trust.json",
    ],
    pathKind: "tree",
    format: "mixed-directory",
    schemaSource: directorySource("core/plugin-manager.ts", "installed manifest, install receipt, backup, multi-source artifacts, and marketplace registry protocols"),
    ...
    identityContract: "Bare pluginId remains the active runtime projection identity; retained artifacts, trust, and backups are marketplace-qualified as {marketplaceId, pluginId, artifactDigest}.",
    siteRules: [
      ...rules(["lib/plugin-install-backups.ts", "lib/plugin-install-records.ts", "lib/plugin-artifact-store.ts", "lib/plugin-trust-store.ts", "lib/plugin-marketplace-sources.ts", "lib/plugin-marketplace-snapshots.ts"], "Backs up, records, retains, or trusts marketplace-qualified plugin packages."),
