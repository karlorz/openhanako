...
## PR #4 review and remediation status (2026-08-05)

The read-only review of [karlorz/openhanako PR #4](https://github.com/karlorz/openhanako/pull/4), exact range `992e640e550741b9d46534b105f6a445ab6d8a7b..6d424bff7aa2ed2d2420449c59f09e29523b085f`, is a **historical, frozen request-changes verdict**. Preserve [`docs/plugins/2026-08-05-pr4-marketplace-review.md`](2026-08-05-pr4-marketplace-review.md) as the immutable severity-ranked review record; do not interpret its old head or verdict as the status of the later remediated branch.

All thirteen findings from that review were subsequently remediated and verified across [Batch 1](2026-08-05-pr4-marketplace-remediation-batch1.md), [Batch 2](2026-08-05-pr4-marketplace-remediation-batch2.md), and [Batch 3](2026-08-05-pr4-marketplace-remediation-batch3.md). The final batch records the consolidated source, test, typecheck, lint, build, isolated Agent lifecycle, packaged macOS install/codesign, and attended Marketplace UI evidence. In particular, the isolated lifecycle passed in both `auto` and `operate`, and the packaged UI smoke covered install, the Manage Plugins Hana-skills row, package disable/enable, uninstall, source cleanup, and unchanged native PluginManager inventory/dropzone.

The remediated branch retains the intended product boundaries: native Hana plugins remain owned by `PluginManager`; Claude marketplace packages remain Hana skills; source-qualified marketplace state, full activation snapshots, package gates, and the dedicated skill uninstall route remain the canonical contracts. No upstream issue is claimed as fixed merely by overlap, and the fork's LAN/auth, build-identity, sync-policy, installer, and sg01 operational behavior remains fork-owned.

Integration remains an attended action after final-head verification. Published fork prerelease tags `v0.421.24-karlorz.3` through `v0.421.24-karlorz.5` are ancestors of the PR branch, so PR #4 must enter `dev` with an ancestry-preserving normal merge commit; do not squash or rebase it. The remediation evidence itself does not authorize or claim that the merge or any sg01 deployment has occurred.
