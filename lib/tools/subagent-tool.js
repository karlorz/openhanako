...
export function createSubagentTool(deps) {
  ...
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      // 工具访问策略（收口）：默认甲（全集 + 拦截）。role 暂固定 execute（OPERATE）；
      // 探索者只读档（role=explore → READ_ONLY）待后缀角色规则定后接。
      ...
      if (!store || !parentSessionPath) {
        ...
        return _syncFallback(deps, params, targetAgentId, parentSessionPath, parentCwd, { inc: () => incActive(parentSessionPath), dec: () => decActive(parentSessionPath) }, toolAccess);
      }
...
async function _syncFallback(deps, params, targetAgentId, _sessionPath, parentCwd, counter, toolAccess) {
